...
func (m *Manager) RotateFileIfNeeded() {
	...
	if err := m.logFile.Close(); err != nil {
		log.Printf("Failed to close log file during rotation: %v", err)
		return
	}
	m.logFile = nil
...
func TestSyslogConnection(protocol, host string) error {
	...
	// Send a test message
	testMsg := fmt.Sprintf("<%d>%s %s: test connection from ariba-connector\n",
		14, time.Now().Format(time.Stamp), "ariba-connector")
